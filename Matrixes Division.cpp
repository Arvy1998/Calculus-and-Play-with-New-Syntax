...
void matrix_divide(long double matrix1[][100], long double matrix2[][100], long long n1, long long m1, long long n2, long long m2) {
	long double matrix3[100][100];
	for (auto a = 0; a < n1; ++a) {
		for (auto b = 0; b < m2; ++b) {
			matrix3[a][b] = 0;
		}
	...
	for (auto i = 0; i < n1; ++i) {
		for (auto j = 0; j < m2; ++j) {
			for (auto k = 0; k < m1; ++k) {
				matrix3[i][j] = matrix3[i][j] + (matrix1[i][k] * (1 / matrix2[k][j]));
				// a / b = a * (1 / b)
			}
		}
	}
...
int main() {
	...
	long double matrix1[100][100];
	long double matrix2[100][100];
	...
	if (m1 != n2) {
		...
		matrix_divide(matrix1, matrix2, n1, m1, n2, m2);
	}
	cin.get();
	return 0;
}
...
Maximum size of value is long double type.
*/
